...
if (isset($_POST['views_data'])) {
    ...
    try {
        ...
        // Add table header
        $colIndex = 'A';
        $sheet->setCellValue('A1', 'ID Clearance');
        ...
        $sheet->setCellValue('M1', 'Created On');
        ...
        foreach ($columns as $column) {
            $sheet->setCellValue('A' . $rowIndex, $column['id_clearance']);
            ...
            $sheet->setCellValue('G' . $rowIndex, $column['houseno']);
            $sheet->setCellValue('H' . $rowIndex, $column['street']);
            $sheet->setCellValue('I' . $rowIndex, $column['brgy']);
            $sheet->setCellValue('J' . $rowIndex, $column['city']);
            $sheet->setCellValue('K' . $rowIndex, $column['municipality']);
            $sheet->setCellValue('L' . $rowIndex, $column['purpose']);
            $sheet->setCellValue('M' . $rowIndex, $column['created_on']);
            $sheet->setCellValue('N' . $rowIndex, $column['created_by']);
            $rowIndex++;
        }
