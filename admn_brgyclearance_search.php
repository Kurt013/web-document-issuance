                        ...
                        <tr>
                            <td>    
                                <form id = "archiveForm" action="" method="post">
                                    ...
                                    <input type="hidden" name="id" value="<?= $userdetails['id'];?>">
                                    <input type="hidden" name="id_clearance" value="<?= $view['id_clearance'];?>">
                                    <button type="submit" id="hiddenSubmitBtn" style="display:none;" name="archive_brgyclearance">Submit</button>
                            <?php 
                                echo $list === 'active' ? 
                                    '<button class="btn btn-danger archive-btn" type="button" style="width: 70px; font-size: 17px; border-radius:30px;" name="archive_brgyclearance"> <i class="fas fa-archive"></i> </button>' :
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    openPopupBtns.forEach((openPopupBtn) => {
        openPopupBtn.addEventListener('click', function () {
            const dataId = this.closest('form').querySelector('input[name="id_clearance"]').value;
            // Store the ID in the form's data-id (or set a hidden input value if necessary)
            archiveForm.querySelector('input[name="id_clearance"]').value = dataId; // Set the correct id_indigency
